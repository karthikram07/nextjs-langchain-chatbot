...
export function IntermediateStep(props: { message: Message }) {
  ...
  return (
    <div className="mr-auto bg-secondary border border-input rounded p-3 max-w-[80%] mb-8 whitespace-pre-wrap flex flex-col">
      <button
        type="button"
        className={cn('text-left flex items-center gap-1', expanded && 'w-full')}
      ...
      >
        <div className={cn('rounded', expanded ? 'max-w-full' : 'transition-[max-width] delay-100')}>
          Input: <code className="max-h-[100px] overflow-auto">{JSON.stringify(action.args)}</code>
        </div>
        <div className={cn('rounded', expanded ? 'max-w-full' : 'transition-[max-width] delay-100')}>
          Output: <code className="max-h-[260px] overflow-auto">{observation}</code>
        </div>
      </div>
    </div>
  );
